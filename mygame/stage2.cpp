...
/* only stage2 function */
static int wayR(int x, int y) {
	if (y == sHei/2 -1.5*GRID) {
		if ((int)(x + STEP) <= (int)(sWid/2 +1.5*GRID)) {
			return 1;
		} else return 0;
	}		
	return 0;
...
int st2_disp( void ) {
...
#if DEBUG
	DrawFormatString( 0, 16, GetColor( 255, 255, 255 ), ( const TCHAR* )"STAGE2 DISPLAY MODE (%d)", elaps );
	...
	//grid cells detect
	if ( mX > (sWid/2 -4*GRID) && mX < ( sWid/2 +3*GRID ) ) {
		if (mY > ( sHei/2 -3*GRID ) && mY < ( sHei/2 +0*GRID )) {
	...
	if ( mX > (sWid/2 -3*GRID) && mX < ( sWid/2 +3*GRID ) ) {
	...
	//waylines
	//DrawLine( sWid/2 -2.5*GRID, sHei/2 -1.5*GRID, sWid/2 +1.5*GRID, sHei/2 -1.5*GRID, colB ); //upper
	for(int i = sWid/2 -2.5*GRID-STEP; i<= sWid/2 +1.5*GRID-STEP; i+=GRID){ DrawExtendGraph(i, sHei/2 -1.5*GRID-STEP, i+GRID, sHei/2 -1.5*GRID+GRID-STEP, bHandle, TRUE); }
	//DrawLine( sWid/2 +1.5*GRID, sHei/2 -1.5*GRID, sWid/2 +1.5*GRID, sHei/2 +1.5*GRID, colB ); //vertical
	DrawExtendGraph(sWid/2 +1*GRID, sHei/2-1*GRID, sWid/2+2*GRID, sHei/2 -0*GRID, bHandle, TRUE);  
	DrawExtendGraph(sWid/2 +1*GRID, sHei/2-0*GRID, sWid/2+2*GRID, sHei/2 +1*GRID, bHandle, TRUE);
	//DrawLine( sWid/2 +1.5*GRID, sHei/2 +1.5*GRID, sWid/2 -1.5*GRID, sHei/2 +1.5*GRID, colB ); //lover
	for(int i = sWid/2 -1.5*GRID-STEP; i<= sWid/2 +1.5*GRID-STEP; i+=GRID){ DrawExtendGraph(i, sHei/2 +1.5*GRID-STEP, i+GRID, sHei/2 +1.5*GRID+GRID-STEP, bHandle, TRUE); }
	DrawExtendGraph(sWid/2 -2*GRID, sHei/2+GRID, sWid/2-GRID, sHei/2+2*GRID, gHandle, TRUE); 


	//cargo & clear window
	//DrawCircle( maru.x, maru.y, GRID/2, maru.col, TRUE );
	if ( maru.x==(sWid/2 -1.5*GRID) && maru.y==(sHei/2+1.5*GRID) ) { 
		//maru.col = colR; 
		...
		DrawFormatString(sWid/2+GRID, sHei/2-GRID, colW, (const TCHAR*)"%d sec", clTm2);

		if(clSw2==0) { clTm2 = elaps; clSw2 = 1; }
	}
	if(clSw2==0) { DrawExtendGraph(maru.x-STEP, maru.y-STEP, maru.x+STEP, maru.y+STEP, cHandle, TRUE); }
	...
	if(clSw2==0) LoadGraphScreen( p1.x-STEP+1, p1.y-STEP+1, "media/bPlus.png", TRUE );
	//DrawCircle( p2.x, p2.y, GRID/2-2, colR, FALSE, 2 );
	if(clSw2==0) LoadGraphScreen( p2.x-STEP+1, p2.y-STEP+1, "media/bPlus.png", TRUE );
	//DrawCircle( m1.x, m1.y, GRID/2-1, colB, FALSE, 2 );
	if(clSw2==0) LoadGraphScreen( m1.x-STEP+1, m1.y-STEP+1, "media/bMinus.png", TRUE );
	//DrawCircle( m2.x, m2.y, GRID/2-2, colB, FALSE, 2 );
	if(clSw2==0) LoadGraphScreen( m2.x-STEP+1, m2.y-STEP+1, "media/bMinus.png", TRUE );
	//DrawCircle( m3.x, m3.y, GRID/2-3, colB, FALSE, 2 );
	if(clSw2==0) LoadGraphScreen( m3.x-STEP+1, m3.y-STEP+1, "media/bMinus.png", TRUE );
